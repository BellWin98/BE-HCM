...
**기록 위치** — `제발하자 > 프로젝트 제발하자 > HCM > 작업 히스토리 > BE-HCM`
([링크](https://app.notion.com/p/3aaa5de8c4668054a818cbb9a015c4e7))

**작성 규칙**

- `BE-HCM` 페이지 본문에 내용을 직접 쓰지 **않습니다**. 작업 단위(이슈/PR)마다 **하위 페이지를 새로 생성**하고
  그 안에 내용을 작성합니다.
- 하위 페이지 제목: `YYYY-MM-DD {작업 제목}` (예: `2026-07-27 FCM 푸시 알림 안정성 개선 (배치 전송·만료 토큰 정리)`).
- 페이지 구성:
  1. **메타 표** — 리포지토리, 이슈 번호/링크, PR 번호/링크, 브랜치, 커밋 해시, 변경 규모(파일 수, +/- 라인).
     연관된 프론트엔드 작업이 있으면 해당 PR 링크도 함께 표기합니다.
  2. **한 줄 요약** — 이 작업이 무엇을 바꿨는지 한 문장으로.
  3. **변경 내용** — 항목별 섹션. *무엇을 바꿨는지*뿐 아니라 **왜 그렇게 했는지(근거·트레이드오프)**를 함께
     적어 나중에 다시 봐도 판단 근거가 남도록 합니다. 보류하기로 한 결정(예: deprecated API 이전 유예)도
     그 사유와 함께 남깁니다.
  4. **테스트** — 추가/갱신한 테스트와 커버한 케이스.
- 커밋 메시지를 그대로 옮기지 말고, 코드/diff를 읽고 맥락과 배경을 담아 일목요연하게 재구성합니다.
- API 키·토큰 등 비밀값은 문서에 옮기지 않습니다.

...
public interface RestRepository extends JpaRepository<Rest, Long> {
    ...
    @Query("SELECT r FROM Rest r WHERE r.workoutRoomMember IN :workoutRoomMembers")
    List<Rest> findAllByWorkoutRoomMembersIn(@Param("workoutRoomMembers") List<WorkoutRoomMember> workoutRoomMembers);
}

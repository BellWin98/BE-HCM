...
/**
 * "나에게 토스 접근·주문 권한이 있는가"를 확인하는 엔드포인트.
 *
...
public class TossAccessController {
    ...
    public ResponseEntity<ApiResponse<TossAccessResponse>> getMyAccess(
        ...
        boolean canTrade = tossAccessChecker.canTrade(member);
        return ResponseEntity.ok(ApiResponse.success(new TossAccessResponse(hasAccess, canTrade)));
    }
}
